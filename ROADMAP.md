...
- [ ] Fire terrain-eating rate/duration matching real WA — landed fire now gradually eats terrain again (`step_fire_patches`, unversioned build on Miyoo .126 as of 2026-07-10; not yet in a numbered release), but the eat-rate/radius and the 5s-per-patch cutoff are a hand-tuned guess, not a measured match to WA. Same failure pattern as the bazooka RE effort above: static RE of WA.exe found no usable constants, and Wine-rig dynamic capture couldn't get a reliable measurement (per-turn camera hard-cuts, short retreat-time capture window, pre-turn input lockout, inconsistent real-time↔game-time ratio). Full history in PROGRESS.txt/STATUS.md — pick up from there, don't re-derive
