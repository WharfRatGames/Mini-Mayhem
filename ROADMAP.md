...
- [x] New "bng" open-air terrain family — 8 new masks (`bng0-7.bin`) of denser, chunkier land masses with rolling hilltops, selected on ~26% of open-air seeds via a new `collage_params` roll (`BNG_SHARE_PCT`); no `terrain.rs`/`StateMsg` change, rides the existing template machinery. Mask library also grown: island 20→28, cavern 8→16, extracted from an expanded MapGEN corpus (100+130 more per type) with `extract_wa_mask.py --nonblack --clean-sprites`. Corpus tooling gained a `--start` append offset + `tools/corpus_progress.py` progress/ETA viewer (v0.5.4.432)
- [x] Map relief tuned up — `RELIEF_COMPRESSION` 1.05→1.00 (it's a multiplier on `y_scale`: higher = more relief). Best `surf_span` of three trial points (289.5 @1.00 vs 280.5 @1.05 vs 283 @0.95); the lever is weak/noisy so the residual ~18px to the ~308 blend target is left alone. `cliff_frac` 0.094, under the traversability guard's 0.20 cap (v0.5.4.433)
